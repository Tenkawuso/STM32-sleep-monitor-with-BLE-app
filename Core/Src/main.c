...
/* UART TX ring buffer state (main.c only) */
...
static void UartTx_Push(const uint8_t *data, uint16_t len)
{
  ...
  for (i = 0U; i < len; i++)
  {
    next = (uint16_t)((s_tx_wr + 1U) & (UART_TX_BUF_SIZE - 1U));
    if (next != s_tx_rd)
    {
      s_uart_tx_buf[s_tx_wr] = data[i];
      s_tx_wr = next;
    }
  }
}
...
static void UartTx_Poll(void)
{
  ...
  len = (uint16_t)((s_tx_wr - s_tx_rd) & (UART_TX_BUF_SIZE - 1U));
  if (len == 0U)
  {
    return; /* buffer empty */
  }
...
/**
 * @brief  UART TX half-completion callback (reuse to clear sending flag).
 */
void HAL_UART_TxHalfCpltCallback(UART_HandleTypeDef *huart)
{
  /* not used, but silence a potential weak-linker warning */
}
